...
- Revision panel with checkbox "Đã sửa" MVP and localStorage persistence
...
|-------|-------|--------|-------|-------------|
| 4.1 | Faculty Approve Action | ✅ Done | 37 | Approve endpoint, RBAC, state transition to SCHOOL_SELECTION_REVIEW |
| 4.2 | Faculty Return Dialog | ✅ Done | 8 new | Return dialog, reason codes, section checkboxes, return target storage |
| 4.3 | Execute Return + Banner | ✅ Done | 43 total | ChangesRequestedBanner, getLatestReturn API |
| 4.4 | Revision Panel | ✅ Done | Created | Checkbox "Đã sửa", localStorage, click-to-scroll, 4 fixes |
| 4.5 | Resubmit Action | ✅ Done | New | Reads return_target from log, SLA reset, 3 fixes |
| 4.6 | Revision PDF Export | ✅ Done | New | PDF generation, download button, 3 fixes |
...
- Conditional "Nộp lại" button enablement
...
### 1. Frontend Tests Path Alias Issue ✅ FIXED
...
**Solution:** Updated vitest.config.ts with:
- `environment: 'jsdom'` for frontend tests
- `environmentMatchGlobs` for node environment on backend tests
- Proper aliases for `@/lib` and `@/components`
- Result: 474 tests passing (up from ~400 before)
...
### 3. E2E Tests Not Implemented ✅ FIXED
...
**Status:** FIXED 2026-01-07
**Solution:** Created `web-apps-e2e/src/faculty-return.spec.ts` with:
- Full return dialog flow tests
- RBAC validation tests
- Form validation tests
- Idempotency tests
- ChangesRequestedBanner tests

### 4. lucide-react Package Installation ✅ ALREADY IN PACKAGE.JSON
...
**Status:** VERIFIED 2026-01-07 - Already in package.json (line 81)
**Lesson:** Check dependencies before installing - it was already there!
...
throw new BadRequestException({
  ...
  error: {
    ...
    message: 'Các section không hợp lệ...', // Vietnamese message
  },
});
```
...
### 4. Checkbox "Đã sửa" Is User-Driven
Story 4.4 confirmed MVP approach: user manually checks "Đã sửa" checkboxes. No automatic hash detection of field changes.
...
|------|-------|--------|-------------|--------|
| Vitest path alias (@) | All frontend | Medium | Fix vitest.config.ts for .tsx files | ✅ Fixed |
| E2E Tests for Return Flow | 4.2 | Low | Add Playwright E2E tests | ✅ Fixed |
...
|-------------|--------|----------|
| Use WorkflowService.transitionState() | ✅ Applied | All Epic 4 transitions use service |
| Populate return_target fields | ✅ Applied | Story 4.2 stores EXPLICIT targets |
| Resubmit reads from workflow_logs | ✅ Applied | Story 4.5 reads return_target |
| Apply IdempotencyInterceptor | ✅ Applied | All POST endpoints use interceptor |
...
**Objective:** PKHCN phân bổ cho hội đồng trường → Hội đồng đánh giá → Submit ONCE → khóa chỉnh sửa.
...
**Action Items:** 3 (All Complete ✅)
1. ✅ Fix vitest.config.ts for @ path alias resolution - DONE
2. ✅ Add E2E tests for return dialog flow - DONE
3. ✅ Verify lucide-react in package.json - VERIFIED
